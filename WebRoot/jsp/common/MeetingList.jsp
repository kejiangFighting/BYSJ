...
<body class="childrenBody">
	<blockquote class="layui-elem-quote">我的预约</blockquote>
		    ...
		    <tbody class="links_content">		   		
						<%	//先查出今日之后的链表显示，再查出今日之前的链表显示
							AdmDaoImpl ad=new AdmDaoImpl();
 							List<Meeting> meetingList=ad.getMeeting(user);
							...
							if(meetingList.size()>0){
								for(Meeting n:meetingList){
									...
									%>
										<td>
											...
											<a class="layui-btn layui-btn-danger layui-btn-mini links_del" href='DeleteUserServlet?MeetingID=<%=n.getID() %>'>
								...
								<% }} %>
				<%
				List<Meeting> List=ad.getOldMeeting(user);
				if(List.size()>0){
				for(Meeting n:List){
					%>
					<tr>
